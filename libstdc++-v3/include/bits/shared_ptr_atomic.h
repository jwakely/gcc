...
# define __cpp_lib_atomic_shared_ptr 201711L
  ...
  template<typename _Tp>
	...
	lock(memory_order __o) const noexcept
	{
	  ...
	  if (__o == memory_order_relaxed || __o == memory_order_consume)
	...
	notify_all() noexcept
	{
      ...
      private:
	mutable __atomic_base<uintptr_t> _M_val;
	...
	_M_refcount._M_swap_unlock(__r._M_refcount, __o);
      ...
      compare_exchange_strong(value_type& __expected, value_type __desired,
			      memory_order __o, memory_order __of) noexcept
      {
	bool __result = true;
